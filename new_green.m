function [val, grad, hess] = new_green(src,targ,zk,kappa,d,pxys,cs,l,ising)

if abs(zk) > 1e-10
    pgreen = @(s,t) chnk.helm2d.green(zk,s,t);
    sgreen = pgreen;
else
...
if ~isempty(ryfar)
    ...
    if nargout > 2
    [val(:,ifar,:), grad(:,ifar,:), hess(:,ifar,:)] =  quasi_dual_sum(rxfar,ryfar,zk,kappa,d);
    elseif nargout > 1
    [val(:,ifar,:), grad(:,ifar,:)] =  quasi_dual_sum(rxfar,ryfar,zk,kappa,d);
    else
    val(:,ifar,:) =  quasi_dual_sum(rxfar,ryfar,zk,kappa,d);
    end
end
