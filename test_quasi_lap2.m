...
[s0,sn] = chnk.lap2dquas.latticecoefs(1:30,d,kappa,l);
...
tic;
[us,grad,hess] = chnk.lap2dquas.green([0;0],targs,kappa,d,s0,sn,l,1);
toc;
...
dkern = kernel(@(s,t) chnk.lap2dquas.kern(s,t,'d',kappa,d,s0,sn,l,1));
dkern.sing = 'smooth';
hkern = kernel(@(s,t) chnk.lap2dquas.kern(s,t,'hilb',kappa,d,s0,sn,l,1));
hkern.sing = 'pv';
...
src.r = [0.2;-1];
rhs = chnk.lap2dquas.kern(src,chnkr,'s',kappa,d,s0,sn,l);
...
%%
kappas = [kappa,linspace(-pi/d,pi/2,20) - 0.3i];
[s0,sn] = chnk.lap2dquas.latticecoefs(1:30,d,kappas,l);

dkern = kernel(@(s,t) chnk.lap2dquas.kern(s,t,'d',kappas,d,s0,sn,l,1));
dkern.sing = 'smooth';
hkern = kernel(@(s,t) chnk.lap2dquas.kern(s,t,'hilb',kappas,d,s0,sn,l,1));
hkern.sing = 'pv';

a = dkern.eval(src,struct('r',targs));

dmats = chunkermat(chnkr,dkern);
hmats = chunkermat(chnkr,hkern);

norm(dmats(1:21:end,:) - dmat)
norm(hmats(1:21:end,:) - hmat)
